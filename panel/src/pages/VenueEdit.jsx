...
export default function VenueEdit() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [venue, provincesData, categoriesData, services, specialists, photos] = await Promise.all([
                venueService.getById(id),
                locationService.getProvinces(),
                locationService.getCategories(),
                serviceService.getVenueServices(id),
                specialistService.getVenueSpecialists(id),
                photoService.getVenuePhotos(id)
            ]);
            ...
            setFormData({
                ...
                is_hygienic: venue.is_hygienic || false,
                ...
                social_links: {
                    phone: venue.social_links?.phone || '',
                    whatsapp: venue.social_links?.whatsapp || '',
                    instagram: venue.social_links?.instagram || '',
                    facebook: venue.social_links?.facebook || '',
                    website: venue.social_links?.website || ''
                },
                ...
                accessibility: {
                    wheelchair: venue.accessibility?.wheelchair || false,
                    parking: venue.accessibility?.parking || false,
                    wifi: venue.accessibility?.wifi || false,
                    air_conditioning: venue.accessibility?.air_conditioning || false
                },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await venueService.update(id, venuePayload);

            await Promise.all([
                serviceService.saveVenueServices(id, formData.services),
                specialistService.saveSpecialists(id, formData.specialists),
                photoService.savePhotoMetadata(id, formData.photos)
            ]);
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center min-h-[600px] animate-fade-in">
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-10 animate-fade-in">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-4xl font-bold text-secondary">Düzenle: {formData.name}</h2>
                    <p className="text-gray-400 font-medium mt-2">İşletme bilgilerini güncelleyin</p>
                </div>
                <div className="flex gap-4">
                    ...
                    <Button onClick={handleSubmit} disabled={saving} className="min-w-[180px]">
                        {saving ? <LoadingSpinner size="sm" /> : <span className="material-symbols-outlined text-[20px] fill-1">save</span>}
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                ...
                                <div className="col-span-2 space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        rows={5}
                                        className="w-full px-5 py-4 rounded-2xl bg-white border border-gray-100 focus:border-primary/30 focus:shadow-lg focus:shadow-primary/5 outline-none transition-all duration-300 text-sm font-medium text-secondary"
                                    />
                                </div>
                            </div>
                        </div>
